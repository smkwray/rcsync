...
fn render_filters_file(cfg: &AppConfig, project: &Project) -> Result<String, String> {
    let mut out = String::from("# Generated by rcsync. Editing this file by hand will make the\n\
    ...
    for pattern in cfg.excludes.iter().chain(project.excludes.iter()) {
        ...
        // A newline inside a pattern would forge extra rules, and rclone would
        // read them as ours. Refuse rather than sanitise.
        if pattern.contains('\n') || pattern.contains('\r') {
            return Err(format!("Exclude pattern contains a line break: {:?}", pattern));
        }
        out.push_str("- ");
...
/// Where a project's bi-sync filters file lives. Stable across runs, because
/// rclone hashes this file's *contents* and compares against what it stored for
/// the previous run — a path that moved would defeat the check it exists for.
fn filters_file_path(project: &Project) -> Result<std::path::PathBuf, String> {
    ...
    // The project name is the registry key everywhere else in this app; keep
    // path separators out of it so it cannot escape this directory.
    let safe: String = project
        .name
        .chars()
        .map(|c| if c.is_ascii_alphanumeric() || c == '-' || c == '_' { c } else { '_' })
        .collect();
    Ok(dir.join(format!("{}.filter", safe)))
}
...
fn bisync_abort_guidance(output: &str) -> Option<&'static str> {
    ...
    let pre_apply_stop = output.contains("too many deletes")
        || output.contains("--max-delete")
        || output.contains("all files were changed")
        || output.contains("filters file has changed");
    pre_apply_stop.then_some(
        "NOTE: this run stopped BEFORE applying any queued change, so nothing was copied or \
         ...
         likely cause — every newly-hidden file looks like a deletion to bisync. Recover with a \
         resync that uses this project's own filters, not the `--force` rclone suggests above \
         (which suppresses the check rather than rebuilding the listings) and not a bare \
         `rclone bisync --resync` (which would rebuild them under a different filter set).",
    )
}
...
mod tests {
    ...
    fn fast_list_goes_to_push_alone() {
        ...
        assert!(
            !has_flag(&check_argv(&cfg, &project, "/tmp/local", "gdrive:proj/x")),
    ...
    fn a_filters_file_selects_exactly_what_the_inline_excludes_select() {
        ...
        let mut inline = build_exclude_args(&cfg);
        inline.extend(project_exclude_args(&project));
    ...
    #[test]
    fn a_pattern_containing_a_line_break_is_refused_not_sanitised() {
        // A newline would forge extra rules into the file, and rclone would read
        // them as ours — silently changing what a bi-sync considers in scope.
        let cfg = test_cfg(vec!["ok/**"]);
        let bad = project_with(vec!["evil/**\n+ **"]);
        assert!(render_filters_file(&cfg, &bad).is_err());
        assert!(render_filters_file(&cfg, &project_with(vec!["fine/**"])).is_ok());
    }
