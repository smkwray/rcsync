...
///
/// Rendered from the same two builders the inline `--exclude` arguments come
/// from, so the two forms cannot describe different sets — that equivalence is
/// the empty-source guard's parity property and is pinned by test against the
/// real rclone binary.
...
mod tests {
    ...
    fn changing_the_filter_set_makes_the_next_bisync_refuse_before_touching_anything() {
        ...
        let survived = a.join("src/keep.txt").exists() && b.join("src/keep.txt").exists();
        let junk_intact = a.join("junk/j.txt").exists();
